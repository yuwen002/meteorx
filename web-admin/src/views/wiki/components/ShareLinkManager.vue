<template>
  <el-dialog v-model="visible" title="分享链接管理" width="800px">
    <div class="share-manager">
      <el-button type="primary" @click="toggleCreate">
        {{ showCreateForm ? '取消创建' : '创建分享链接' }}
      </el-button>

      <div v-if="showCreateForm" class="create-form">
        <el-divider>新建分享链接</el-divider>
        <el-form :model="createForm" label-width="120px" inline>
          <el-form-item label="访问密码">
            <el-input v-model="createForm.password" placeholder="留空则无需密码" clearable style="width: 180px" />
          </el-form-item>
          ...
          <el-form-item label="最大浏览次数">
            <el-input-number v-model="createForm.max_views" :min="1" :max="10000" placeholder="不限" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" :loading="submitting" @click="submitCreate">确定创建</el-button>
            <el-button @click="toggleCreate">取消</el-button>
          </el-form-item>
        </el-form>
      </div>
      ...
      <el-table :data="shareLinks" style="width: 100%; margin-top: 16px">
        <el-table-column prop="token" label="链接Token" min-width="120">
          <template #default="{ row }">
            <el-link type="primary" @click="copyLink(row.token)">{{ row.token }}</el-link>
          </template>
        </el-table-column>
...
import { listShareLinks, createShareLink, deleteShareLink, type ShareLink } from '@/api/modules/wiki'
...
function toggleCreate() {
  if (showCreateForm.value) {
    ...
    createForm.value = { password: '', expires_at: '', max_views: undefined }
