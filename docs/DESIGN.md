...
- 审计日志不可篡改
- 支持 CSV 导出和多维度查询
- **批量处理优化**：使用 `AuditBatchProcessor` 批量写入数据库（100条/5秒），性能提升10-100倍
- **Context传递**：异步审计日志使用带超时的Context（10秒），避免goroutine泄漏
...
- `/health` - 基础健康检查（负载均衡器使用）
- `/health/ready` - 深度就绪检查（K8s就绪探针使用）
  - 检查数据库连接状态
  - 检查Redis连接状态
  - 返回详细JSON健康状态
