...
| **性能优化** | 审计日志异步批量写入（缓冲区 100 条 / 5 秒刷新） |
...
| `GET` | `/audit/logs/export?format=csv` | 导出审计日志为 CSV |
...
- **登录失败锁定**：连续失败 5 次后账号锁定 30 分钟，前端显示剩余尝试次数
- **密码策略**：默认要求 8 位以上，包含大小写字母、数字和特殊字符
- **接口限流**：每个 IP 每分钟最多 100 个请求，防止 DDoS 攻击
- **审计日志导出**：支持按筛选条件导出 CSV，带 BOM 支持 Excel 中文
- **异步批量写入**：审计日志缓冲区 100 条或 5 秒自动刷新，提升性能
